(function relayEventsFactory(root, factory) {
...
})(typeof window === "undefined" ? null : window, function createRelayEvents() {
  ...
  function resolveEventStages(event, entries) {
    ...
    var byId = new Map((entries || []).map((entry) => [entry.id, entry]));
    return event.stageIds.map((id) => byId.get(id)).filter(Boolean);
  }
  ...
  function formatPeriod(event, locale) {
    ...
    return (
      formatter.format(toDate(event.startsOn)) +
      " ~ " +
      formatter.format(toDate(event.endsOn))
    );
  }
