(function relayDailyV2Factory(root, factory) {
...
})(typeof window !== "undefined" ? window : null, function createRelayDailyV2() {
  "use strict";
  ...
  function isEligible(entry, rules) {
    ...
    var statuses = (rules && rules.eligibleStatuses) || defaultRules().eligibleStatuses;
  ...
  function availableWithCooldown(pool, recentIds, slotCount, minMechanics) {
    ...
    for (var i = 0; i < cooledNewestFirst.length; i += 1) {
      ...
      var remaining = pool.filter(function (entry) { return !trial.has(entry.id); });
      if (remaining.length < slotCount) break;
      if (mechanicCount(remaining) < minMechanics) break;
      keepCool = trial;
    }
    return pool.filter(function (entry) { return !keepCool.has(entry.id); });
  }
  ...
  function pickRoute(pool, dateKey, rules, recentIds) {
    ...
    var available = availableWithCooldown(pool, recentIds, config.slotCount, config.minMechanicsPerDay);
    ...
    function tryPick(requireNewMechanic) {
      for (var i = 0; i < ordered.length && selected.length < config.slotCount; i += 1) {
        var entry = ordered[i];
        if (selected.some(function (item) { return item.id === entry.id; })) continue;
        var mechanic = mechanicOf(entry);
        if (requireNewMechanic && mechanic && mechanics.has(mechanic)) continue;
        selected.push(entry);
        if (mechanic) mechanics.add(mechanic);
      }
    }
    ...
    if (selected.length < config.slotCount) {
      throw new Error("eligible pool cannot fill " + config.slotCount + " slots on " + dateKey);
    }
    if (mechanics.size < config.minMechanicsPerDay) {
      throw new Error("eligible pool cannot provide " + config.minMechanicsPerDay + " mechanics on " + dateKey);
    }
    return selected.slice(0, config.slotCount);
  }

  function eligiblePool(catalog, rules) {
    return (catalog || []).filter(function (entry) { return isEligible(entry, rules); });
  }
  ...
  function buildChallenge(catalog, dateKey, rules, recentByDay) {
    ...
    for (var i = 1; i <= config.cooldownDays; i += 1) {
      ...
      (previous || []).forEach(function (id) { recentIds.push(id); });
    }
    var route = pickRoute(eligiblePool(catalog, config), key, config, recentIds);
    return {
      ...
      rankedWrites: false,
      stages: route.map(function (entry) {
        return {
          id: entry.id,
          title: entry.title,
          path: entry.path,
  ...
  function simulateRange(catalog, startDateKey, days, rules) {
    ...
    for (var i = 0; i < days; i += 1) {
      ...
      recentByDay[dateKey] = challenge.stages.map(function (stage) { return stage.id; });
  ...
  function historyBefore(catalog, dateKey, rules) {
    ...
    var simulated = simulateRange(catalog, start, config.cooldownDays, config);
    var recentByDay = {};
    for (var i = 0; i < simulated.length; i += 1) {
      recentByDay[simulated[i].dateKey] = simulated[i].stages.map(function (stage) {
        return stage.id;
      });
    }
    return recentByDay;
