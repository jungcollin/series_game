(function relayGalleryFilterFactory(root, factory) {
...
})(typeof window !== "undefined" ? window : null, function createRelayGalleryFilter() {
  "use strict";
  ...
  function readSearchParams(search) {
    var params = new URLSearchParams(typeof search === "string" ? search.replace(/^\?/, "") : "");
    return {
      q: params.get("q") || "",
  ...
  function matchesQuery(entry, query) {
    ...
    if (!q) return true;
    var creator = entry.creator && entry.creator.name ? entry.creator.name : "";
    var haystack = [
      entry.id,
      entry.title,
      entry.genre,
  ...
  function applyFilters(entries, options) {
    ...
    var filtered = (entries || []).filter(function (entry) {
      ...
      if (options.mechanic && (!entry.review || entry.review.mechanic !== options.mechanic)) return false;
    ...
    if (sort === "name") {
      sorted.sort(function (a, b) { return String(a.title || "").localeCompare(String(b.title || ""), "ko"); });
    } else if (sort === "newest") {
