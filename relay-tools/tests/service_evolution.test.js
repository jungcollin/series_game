...
const { buildCatalog, checkCatalogSync } = require("../scripts/build_catalog.js");
...
test("local favorites and recent plays survive broken JSON", () => {
  const storage = {
    data: { "olr-favorites-v1": "{not-json", "olr-recent-plays-v1": "{also-bad" },
  ...
  assert.deepEqual(LocalStore.dropMissing(["missing", "galaxy-boss"], ["galaxy-boss"]), ["galaxy-boss"]);
});
...
test("result policy keeps practice and writes-off out of official submits", () => {
  ...
  assert.equal(ResultPolicy.classifyRun({
    mode: "practice",
    challengeId: today,
...
test("gallery filters preserve query state and newest uses publishedAt", () => {
  const entries = [
    { id: "a", title: "Alpha", genre: "Arcade", publishedAt: "2026-01-02T00:00:00Z", review: { mechanic: "dodge" } },
    { id: "b", title: "Beta", genre: "Puzzle", publishedAt: "2026-02-02T00:00:00Z", review: { mechanic: "memory" } },
  ];
  ...
  assert.equal(GalleryFilter.writeSearchParams({ q: "beta", sort: "newest", fav: true }), "?q=beta&sort=newest&fav=1");
});

test("challenge ranks never reward a faster fail over a later fail with more clears", () => {
  const rows = ChallengeRank.withRanks([
    { subject: "a", finished_all_clear: false, clear_count: 0, duration_sec: 1, created_at: "2026-01-01T00:00:00Z" },
    { subject: "b", finished_all_clear: false, clear_count: 2, duration_sec: 40, created_at: "2026-01-01T00:00:01Z" },
    { subject: "c", finished_all_clear: true, clear_count: 5, duration_sec: 90, created_at: "2026-01-01T00:00:02Z" },
  ]);
...
test("daily v2 28-day simulation stays inside the eligible pool", () => {
  ...
  const rules = JSON.parse(fs.readFileSync(path.join(repoRoot, "content", "daily-rules.json"), "utf8"));
  const challenges = DailyV2.simulateRange(catalog.entries, "2026-09-01", 28, rules);
  ...
  const withHistory = DailyV2.buildChallenge(catalog.entries, "2026-09-02", rules, {
    "2026-09-01": previous.stages.map((stage) => stage.id),
  });
  const withoutHistory = DailyV2.buildChallenge(catalog.entries, "2026-09-02", rules);
  assert.notDeepEqual(
    withHistory.stages.map((stage) => stage.id),
    withoutHistory.stages.map((stage) => stage.id),
  );
  const overlap = withHistory.stages.filter((stage) => previous.stages.some((item) => item.id === stage.id));
  assert.equal(overlap.length, 0);
  const simulated = DailyV2.simulateRange(catalog.entries, "2026-09-01", 10, rules);
  for (let index = 1; index < simulated.length; index += 1) {
    const previousIds = new Set(simulated[index - 1].stages.map((stage) => stage.id));
    const repeats = simulated[index].stages.filter((stage) => previousIds.has(stage.id));
    assert.equal(repeats.length, 0, "consecutive days must not reuse yesterday's stages");
  }
  const r1 = DailyV2.buildChallenge(catalog.entries, "2026-09-08", Object.assign({}, rules, { revision: 1 }));
  const r2 = DailyV2.buildChallenge(catalog.entries, "2026-09-08", Object.assign({}, rules, { revision: 2 }));
...
test("catalog hash is stable and author approvals are rejected", () => {
  ...
  fs.mkdirSync(path.join(fixture, "community-stages", "bad-stage"), { recursive: true });
  fs.mkdirSync(path.join(fixture, "content"), { recursive: true });
  fs.writeFileSync(path.join(fixture, "community-stages", "bad-stage", "index.html"), "<html></html>");
  fs.writeFileSync(path.join(fixture, "community-stages", "bad-stage", "meta.json"), JSON.stringify({
    id: "bad-stage",
    title: "Bad",
    description: "nope",
    genre: "Test",
  ...
  fs.writeFileSync(path.join(fixture, "content", "reviews.json"), JSON.stringify({ stages: {} }));
...
test("stage packages are content-hashed and stay put when another stage changes", () => {
  ...
  assert.equal(first.hash, second.hash);
  assert.ok(first.files.some((file) => file.path.endsWith("galaxy-boss/index.html")));
  assert.ok(first.files.some((file) => file.path.endsWith("sdk/v2/relay-sdk.js")));
});
...
test("SDK v2 ignores the wrong instance or token", () => {
  ...
  assert.equal(SdkV2.acceptHostMessage(messages[0], { token: "token-a", instanceId: "inst-1" }), true);
  assert.equal(SdkV2.acceptHostMessage(messages[0], { token: "token-b" }), false);
  host.dispose();
  host.ready({ id: "galaxy-boss" });
...
test("v2 ranked writes stay closed in default config and isolation remains sandbox-only", () => {
  const config = fs.readFileSync(path.join(repoRoot, "relay-api/src/config.ts"), "utf8");
  const home = fs.readFileSync(path.join(repoRoot, "index.html"), "utf8");
  assert.match(config, /rankedWritesV2: readFlag\(env.RANKED_WRITES_V2, false\)/);
  assert.match(config, /analyticsIngest: readFlag\(env.ANALYTICS_INGEST, false\)/);
...
test("ops migration keeps v1 tables and adds reports, audit, and v2 runs", () => {
  const sql = fs.readFileSync(path.join(repoRoot, "relay-api/migrations/0002_community_ops.sql"), "utf8");
...
test("analytics excludes technical failures from difficulty rates", () => {
  ...
  assert.equal(Analytics.shouldSend({ name: "stage_fail" }, { optOut: true }), false);
});
